...
export default function renderSheduleBlocks(
	renderData = [],
	teacherMode = false
) {
	...
	renderData.forEach((el, key) => {
		...
		arr.push(
			<Card className="tw mb10add" key={id}>
				<Div>
					...
					>
						<span className="type scheduleSpan inlineFlex">
							<Icon16InfoOutline className="iconPadding" />
							{el.type}
						</span>
						...
						{!teacherMode && (
							<span
								className="teacher scheduleSpan inlineFlex"
		...
		if (
			...
			arr.push(
				<Card key={id + 10} style={{ height: 20, marginBottom: 10 }} />
			);
	});
