  ...
  useEffect(() => {
    ...
    if (renderButtons) {
      ...
      renderShedule(
        ...
        false,
        JSON.parse(localStorage.getItem("group")).id === 0 ? true : false
      );
    }
        ...
        if (Number(group) === 0) {
          authorizedAPI("getSheduleForTeacher", {}).then((data) => {
            if (
              ...
              refreshToken("getSheduleForTeacher", {}).then((data) => {
                if (data.response) {
                  let i = 0;
  ...
  const renderSearch = useCallback(
    (dataForRender) => {
      ...
      dataForRender.forEach((el) => {
        arr.push(
          <Cell onClick={() => setGroupFunction(el.id, el.name)} key={el.id}>
            {el.name}
          </Cell>
        );
      });
    ...
    <Fragment>
      ...
      <Group>
        {loaded ? (
          ...
          loader && <Spinner size="medium" style={{ margin: "20px 0" }} />
        )}
      </Group>
    </Fragment>
  );
