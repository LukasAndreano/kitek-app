...
import { Icon24CameraOutline, Icon56CameraOutline } from "@vkontakte/icons";
		...
		<Group>
			...
			<FormItem>
				<File
					...
					before={!disabled && <Icon24CameraOutline />}
					...
					onChange={(e) => {
						...
						if (
							...
							authorizedAPIFiles("updateProfilePicture", form)
								.then((data) => {
									if (data.response) {
										dispatch(updateAvatar(data.url));
										dispatch(
											setSnackbar({
												text: "Аватар успешно обновлён!",
												success: true,
											})
										);
										props.closeModal();
										...
										if (
											...
											refreshTokenWithFileUpload(
												...
												.catch(() => {
													...
													dispatch(
														setSnackbar({
															text: "Доступные форматы для загрузки: png, jpeg",
															success: false,
														})
													);
												});
				...
				>
					{disabled ? "Обработка..." : "Выберите фотографию"}
				</File>
			</FormItem>
		</Group>
	);
